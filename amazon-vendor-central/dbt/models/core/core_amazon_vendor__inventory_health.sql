-- core_amazon_vendor__inventory_health - the operational scorecard Amazon grades you on,
...
-- Depends on: stg_amazon_vendor__inventory, .sales
--
...
spine AS (
    ...
    FROM {{ ref('stg_amazon_vendor__inventory') }}
    UNION DISTINCT
    SELECT amazon_vendor, marketplace, distributor_view, selling_program, asin, date
    FROM daily_sales
),
...
FROM {{ ref('stg_amazon_vendor__inventory') }} i
