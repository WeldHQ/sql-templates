-- amazon_vendor_sales_by_asin - sales, traffic and Amazon's margin on one row per
...
-- Depends on: staging.amazon_vendor.sales, .traffic, .margin
--
...
SELECT
    ...
    -- see staging.amazon_vendor.margin.
...
FROM sales s
LEFT JOIN {{staging.amazon_vendor.traffic}} t
       ON  t.amazon_vendor = s.amazon_vendor
       AND t.marketplace   = s.marketplace
       AND t.date          = s.date
       AND t.asin          = s.asin
LEFT JOIN {{staging.amazon_vendor.margin}} m
