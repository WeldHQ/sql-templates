-- amazon_vendor_forecast_vs_actuals - Amazon's demand forecast against what actually
-- shipped, per ASIN per forecast window.
--
-- Grain: forecast window x vendor x marketplace x ASIN.
-- Depends on: staging.amazon_vendor.forecast, .sales
--
...
WITH actuals AS (
    ...
    FROM {{staging.amazon_vendor.forecast}} f
    ...
    LEFT JOIN {{staging.amazon_vendor.sales}} s
