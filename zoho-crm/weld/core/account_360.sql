...
-- Depends on: staging.zoho_crm.account, .contact, .user, .note,
...
-- connector - see core.zoho_crm.rep_activity for why. last_note_at is the closest
...
deals AS (
    ...
    FROM {{core.zoho_crm.deal_pipeline}}
    WHERE account_id IS NOT NULL
    GROUP BY 1, 2
),
...
note_targets AS (
    ...
    FROM {{staging.zoho_crm.account}} a
    UNION ALL
    SELECT d.zoho_org, d.account_id, d.deal_id
    FROM {{core.zoho_crm.deal_pipeline}} d
    WHERE d.account_id IS NOT NULL
    UNION ALL
    SELECT c.zoho_org, c.account_id, c.contact_id
    FROM {{staging.zoho_crm.contact}} c
