-- core_amazon_seller__product_sales_over_time - the sales equation at line grain, event
...
-- Depends on: stg_amazon_seller__orders, .returns, .fba_returns, .listings
--
...
-- row type in BI.
--
-- RECONCILING TO core_amazon_seller__sales_over_time. Sum net_sales over the SALE
...
WITH sales AS (
    SELECT
        ...
        o.is_cancelled
    FROM {{ ref('stg_amazon_seller__orders') }} o
),
...
mfn_returns AS (
    ...
    FROM {{ ref('stg_amazon_seller__returns') }} r
),
...
avg_price AS (
    ...
    FROM {{ ref('stg_amazon_seller__orders') }}
    WHERE quantity > 0
    GROUP BY 1, 2, 3
),
