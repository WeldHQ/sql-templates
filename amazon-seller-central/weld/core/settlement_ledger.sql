-- amazon_seller_settlement_ledger - the settlement report turned into a P&L.
--
-- Grain: settlement date x seller x marketplace x order x SKU. Currency: the
-- settlement's own.
-- Depends on: staging.amazon_seller.settlement
--
...
--
-- HOW IT WORKS. staging.amazon_seller.settlement classifies every line into a
...
-- core.amazon_seller.asin_profitability handles the allocation.
...
FROM {{staging.amazon_seller.settlement}} s
