-- amazon_seller_asin_profitability - contribution margin per SKU per day, after
-- Amazon's fees, returns and your COGS.
--
-- Grain: day x seller x marketplace x ASIN x SKU. Currency: the marketplace's.
-- Depends on: core.amazon_seller.product_sales_over_time,
--             core.amazon_seller.settlement_ledger,
--             staging.amazon_seller.fba_reimbursements, .sku_cost
--
...
fees AS (
    ...
    FROM {{core.amazon_seller.settlement_ledger}}
...
reimbursements AS (
    ...
    FROM {{staging.amazon_seller.fba_reimbursements}}
    GROUP BY 1, 2, 3
),
...
SELECT
    ...
    -- COGS, from staging.amazon_seller.sku_cost, valued at the cost that was in
...
LEFT JOIN {{staging.amazon_seller.sku_cost}} c
