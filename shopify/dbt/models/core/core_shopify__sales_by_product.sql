...
-- Depends on: stg_shopify__order, .order_line, .product
--
...
FROM {{ ref('stg_shopify__order_line') }} l
-- shopify_store in every join key: order IDs are only unique within a store.
JOIN orders o USING (shopify_store, order_id)
LEFT JOIN {{ ref('stg_shopify__product') }} p USING (shopify_store, product_id)
-- Gift cards are deferred revenue, not product revenue. stg_shopify__order_line
